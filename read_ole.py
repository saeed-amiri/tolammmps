...
# def slice_df(i):
    ind_i = atoms.Bonds_df.iloc[i]['ai']
    typ_i = atoms.Atoms_df.iloc[ind_i]['typ']
    if typ_i < 5:
        ind_j = atoms.Bonds_df.iloc[i]['aj']
        typ_j = atoms.Atoms_df.iloc[ind_j]['typ']
        if typ_j < 5:
...
atoms = BODY()
atoms.read_body()
atoms_df = atoms.Atoms_df[atoms.Atoms_df['typ']<5]
bonds = []
for i in range(ole.NBonds):
    ind_i = atoms.Bonds_df.iloc[i]['ai']
    typ_i = atoms.Atoms_df.iloc[ind_i]['typ']
    if typ_i < 5:
        ...
        typ_j = atoms.Atoms_df.iloc[ind_j]['typ']
        if typ_j < 5:
            bonds.append([atoms.Bonds_df.iloc[i]['typ'],atoms.Bonds_df.iloc[i]['ai'], atoms.Bonds_df.iloc[i]['aj']])
bonds_df = pd.DataFrame(bonds, columns=['typ', 'ai', 'aj'])
bonds_df.index +=1
